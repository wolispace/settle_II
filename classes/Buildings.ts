		...
		for (let i = 0; i < buildingTypes[this.buildingIndex].fabrications.length; i++) {
			...
			if (fabrication.input == null) {
				...
				if (fabrication.output.type == 'resource') {
					fabricationOutputObj[fabrication.output.value as any] = 1;
			...
			} else if (this.hasResources(fabrication.input) && this.hasOutputFeedSpace(fabrication.output.value as any)) {
				if (fabrication.output.type == 'resource') {
