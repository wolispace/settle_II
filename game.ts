...
const tempAddHouseButton = document.querySelector('#tempAddHouseButton');
...
function init() { 
	...
	console.log(`About to start the render and tick workers`);
    ...
    playStateArray[PLAYER_STATE_ARRAY_INDEXES.SELECTED_BUILDING_TYPE] = -1;
    playStateArray[PLAYER_STATE_ARRAY_INDEXES.ZOOM] = 1;
    ...
    gameCanvas.addEventListener('click', (e)=>{
        ...
        const currentBuildingIdx = Atomics.load(playStateArray, PLAYER_STATE_ARRAY_INDEXES.SELECTED_BUILDING_TYPE);
        if (currentBuildingIdx != -1) {
            ...
            if (allCellsAreValid) {
				...
				let message = {
					roomName: (window as any).roomName,
    ...
    gameCanvas.addEventListener('wheel', (e)=>{
        let existingZoomLevel = Atomics.load(playStateArray, PLAYER_STATE_ARRAY_INDEXES.ZOOM);
        // existingZoomLevel += e.wheelDeltaY/100;
		existingZoomLevel -= e.deltaY / 100;
        // arbitrary values currently
        if (existingZoomLevel > 10 || existingZoomLevel < 0.3) {
            return;
        }
        Atomics.store(playStateArray, PLAYER_STATE_ARRAY_INDEXES.ZOOM, existingZoomLevel);
    })
    ...
    tempAddWoodcutterButton.addEventListener('click', ()=>{
        ...
        Atomics.store(playStateArray, PLAYER_STATE_ARRAY_INDEXES.SELECTED_BUILDING_TYPE, 0);
    })
	...
	tempAddStonecutterButton.addEventListener('click', ()=>{
		Atomics.store(playStateArray, PLAYER_STATE_ARRAY_INDEXES.SELECTED_BUILDING_TYPE, 2);
	});

	tempAddHouseButton.addEventListener('click', ()=>{
		Atomics.store(playStateArray, PLAYER_STATE_ARRAY_INDEXES.SELECTED_BUILDING_TYPE, 3);
	});
    ...
    roomForm.addEventListener('submit', (e)=>{
        e.preventDefault();
		(window as any).roomName = roomValue.value
		jerver.send('joinRoom',{
			roomName: (window as any).roomName
		}).then((result)=>{
			console.log({result})
