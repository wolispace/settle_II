...
import { MAX_MOVABLES, NUM_EXTRA_BITS, MAX_SCHEDULE_DURATION_MS, TICK_PERIOD_MS, MAP_WIDTH, MAP_HEIGHT, DIRECTIONS } from './defs/constants.js';
    ...
    do() {
        if (this.actionType == MOVE_ACTION) {
            ...
            tc.taskQueue.addTask(tc.taskQueue.getTickInFuture(villagerTask.fabricationSet.durationInMs / TICK_PERIOD_MS), new Task((i) => {
                ...
                villagerTask.source.addToOutfeedResources(villagerTask.fabricationSet.output.value);
                // }
            ...
            // let resource = tc.resourceStacks.addResource(dispenser.resourceId, newX, newY, villagerTask.source);
            // clear the quest of the movable, but keep their task intact
            movable.clearQuestOnly();
            movable.quest = [
                // 5) walk to building
...
tc.doTaskMatchmake = (tickToAssignTo) => {
    tc.taskQueue.addTask(tickToAssignTo, new Task((i) => {
        ...
        if (villagerTask instanceof ResourceRequest) {
            ...
            if (resource == null) {
                const err = `This case should never happen because a ResourceRequest should never be picked up unless there is available resources`;
                console.error(err);
                throw new Error(err);
                // return;
            }
            ...
            if (movable == null) {
                const err = `This case should never happen because a ResourceRequest should never be picked up unless there is an idle villager`;
                console.error(err);
                throw new Error(err);
                // return;
            }
            ...
            // this would be redundant if we had two separate arrays for knownMovables and idleMovables
            if (movable == null) {
                const err = `This case should never happen because a BuildRequest should never be picked up unless there is an idle villager`;
                console.error(err);
                throw new Error(err);
                // return;
            }
            ...
            // this would be redundant if we had two separate arrays for knownMovables and idleMovables
            if (movable == null) {
                const err = `This case should never happen because a FabricationRequest should never be picked up unless there is an idle villager`;
                console.error(err);
                throw new Error(err);
                // return;
            }
            ...
            // this would be redundant if we had two separate arrays for knownResources and availableResources
            if (dispenser == null) {
                const err = `This case should never happen because a DispenserFetchRequest should never be picked up unless there is an available dispenser`;
                console.error(err);
                throw new Error(err);
                // return;
            }
            ...
            // this would be redundant if we had two separate arrays for knownMovables and idleMovables
            if (movable == null) {
                const err = `This case should never happen because a DispenserFetchRequest should never be picked up unless there is an idle villager`;
                console.error(err);
                throw new Error(err);
                // return;
            }
...
self.onmessage = e => {
    ...
    tc.resourceStacks.addResource(2, 3, 0);
    tc.resourceStacks.addResource(2, 3, 0);
    tc.resourceStacks.addResource(2, 10, 10);
    tc.resourceStacks.addResource(2, 10, 12);
    tc.resourceStacks.addResource(2, 11, 0);
    tc.resourceStacks.addResource(2, 12, 1);
    tc.resourceStacks.addResource(2, 13, 2);
    tc.resourceStacks.addResource(2, 15, 2);
    tc.resourceStacks.addResource(2, 8, 0);
    tc.resourceStacks.addResource(2, 9, 0);
    tc.resourceStacks.addResource(1, 3, 4);
    tc.resourceStacks.addResource(1, 2, 5);
    tc.resourceStacks.addResource(1, 1, 6);
    tc.resourceStacks.addResource(1, 3, 10);
    // tc.resourceStacks.addResource(1, 4, 11);
    // tc.resourceStacks.addResource(1, 5, 12);
    tc.resourceStacks.addResource(2, 7, 0);
    tc.resourceStacks.addResource(0, 20, 5);
    tc.resourceStacks.addResource(0, 21, 5);
    tc.resourceStacks.addResource(0, 22, 5);
    tc.resourceStacks.addResource(0, 23, 5);
    tc.resourceStacks.addResource(0, 24, 5);
    tc.resourceStacks.addResource(0, 25, 5);
    tc.resourceStacks.addResource(0, 26, 5);
    tc.resourceStacks.addResource(0, 27, 5);
    tc.resourceStacks.addResource(0, 28, 5);
    tc.resourceStacks.addResource(0, 29, 5);
    tc.resourceStacks.addResource(0, 30, 5);
    tc.resourceStacks.addResource(0, 20, 6);
    tc.resourceStacks.addResource(0, 21, 6);
    tc.resourceStacks.addResource(0, 22, 6);
    tc.resourceStacks.addResource(0, 23, 6);
    tc.resourceStacks.addResource(0, 24, 6);
    tc.resourceStacks.addResource(0, 25, 6);
    tc.resourceStacks.addResource(0, 26, 6);
    tc.resourceStacks.addResource(0, 27, 6);
    tc.resourceStacks.addResource(0, 28, 6);
    tc.resourceStacks.addResource(0, 29, 6);
    tc.resourceStacks.addResource(0, 30, 6);
